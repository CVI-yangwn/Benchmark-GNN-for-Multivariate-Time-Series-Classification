...
for key, data in data_dic.items():
    ...
    ## ---------------------------  cal ex cls
    # sum(data)*size - sigma(other)
    # t = data_dic.copy()
    # t.pop(key)
    # # convert list[tensor,] to tensor(cases, dimensions, lengths)
    # stacked_tensor = torch.stack(data, dim=0)  
    # data = torch.sum(stacked_tensor, dim=0)/cases  # sum all of cases and normalize
    # for _, other_cls in t.items():
    #     other_cases = len(other_cls)
    #     other_cls = torch.stack(other_cls, dim=0)  
    #     other_cls = torch.sum(other_cls, dim=0)/other_cases
    #     dis = torch.abs(data - other_cls)  # tensor(dimensions, lengths)
    #     sums += torch.sum(dis, dim=1)  # tensor(dimensions,)
